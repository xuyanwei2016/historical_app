<template>
	<view class="uni-tab-bar">
				...
				<scroll-view id="tab-bar" class="uni-swiper-tab" scroll-x :scroll-left="scrollLeft">
					<view v-for="(tab,index) in tabBars" :key="tab.id" :class="['swiper-tab-list',tabIndex==index ? 'active' : '']"
					 :id="tab.id" :data-current="index" @click="tapTab(index)">
						<view class="wordCont" style="color:#fff">{{tab.name}}{{tabIndex}}
		...
		<swiper :current="tabIndex" class="swiper-box" duration="300" @change="changeTab" v-bind:style="[{'paddingBottom':megMarginBottom,height: scollHeight}]">
			...
			<swiper-item v-for="(tab,index1) in tabBars" :key="index1">
				<scroll-view class="list" scroll-y @scrolltolower="loadMore(index1)">
					...
					<block v-for="(newsitem,index2) in bookData" :key="index2" v-if="tabIndex==0">
						<view class="bookCss" @longtap="longClick(newsitem)">

							<view class="orderCont">
								<view class="checkAll" v-if="isCheckBtn&&tabIndex==0" @tap="clickChang(newsitem)">
								...
								<view class="wordCon" v-bind:style="{width:megWidth[tabIndex]}">
									<view class="title" @tap="linkDetail('/pages/home/bookDetails?id=',newsitem.id,newsitem)">
									...
									<view class="price">￥<text>{{newsitem.isPay?newsitem.isPay:newsitem.price}}</text></view>
					...
					<block v-if="tabIndex==1" v-for="(newsitem,index3) in contentData" :key="index3">
						<view class="articleCss" @longtap="longClick(newsitem)">

							<view class="orderCont">
								<view class="checkAll" v-if="isCheckBtn&&tabIndex==1" @tap="clickChang(newsitem)">
									<view class="noCheck" v-if="!newsitem.isChecked"></view>
									<image src="../../static/images/books/xuanze.png" mode="" style="height: 38upx; width: 38upx;" v-if="newsitem.isChecked"></image>
								</view>
								<view class="wordCon" v-bind:style="{width:megWidth[tabIndex]}">
									...
									<view class="introduce" v-if="newsitem.intro&&newsitem.intro!=''">{{newsitem.intro}}</view>
									<view class="price">￥<text>{{newsitem.isPay?newsitem.isPay:newsitem.price}}</text></view>
									<view class="name" v-if="newsitem.bookName&&newsitem.bookName!=''">
										<navigator :url="'/pages/home/bookDetails?id='+newsitem.bookid" hover-class="none">《{{newsitem.bookName}}》</navigator><text>来源
											&gt;</text>
									</view>
								</view>
					...
					<block v-if="tabIndex==2" v-for="(newsitem,index4) in imageData" :key="index4">
						...
						<view class="imagesCss" @longtap="longClick(newsitem)">

							<view class="orderCont">

								<view class="checkAll" v-if="isCheckBtn&&tabIndex==2" @tap="clickChang(newsitem)">
									<view class="noCheck" v-if="!newsitem.isChecked"></view>
									<image src="../../static/images/books/xuanze.png" mode="" style="height: 38upx; width: 38upx;" v-if="newsitem.isChecked"></image>
								</view>
								...
								<view v-bind:style="{width:megWidth[tabIndex]}">
									<view class="radioImg">
										...
										<image :src="imagePath+newsitem.path" mode="" style="width: 100%;height:350upx;"></image>
					...
					<block v-if="tabIndex==3" v-for="(newsitem,index5) in autioData" :key="index5">
						...
						<view class="radioCss" @longtap="longClick(newsitem)">

							<view class="orderCont">
								<view class="checkAll" v-if="isCheckBtn&&tabIndex==3" @tap="clickChang(newsitem)">
									<view class="noCheck" v-if="!newsitem.isChecked"></view>
									<image src="../../static/images/books/xuanze.png" mode="" style="height: 38upx; width: 38upx;" v-if="newsitem.isChecked"></image>
								</view>

								<view class="radioImg">
									<!-- <image src="../../static/images/home/book1.png" mode="" style="width: 146upx;height:146upx;"></image> -->
					...
					<block v-if="tabIndex==4" v-for="(newsitem,index4) in videoData" :key="index4">
						...
						<view class="imagesCss" @longtap="longClick(newsitem)">
							...
							<view class="orderCont">
								...
								<view class="checkAll" v-if="isCheckBtn&&tabIndex==4" @tap="clickChang(newsitem)">
									<view class="noCheck" v-if="!newsitem.isChecked"></view>
									<image src="../../static/images/books/xuanze.png" mode="" style="height: 38upx; width: 38upx;" v-if="newsitem.isChecked"></image>
								</view>
					...
					<block v-if="tabIndex==5" v-for="(newsitem,index6) in otherData" :key="index6">
						...
						<view class="otherCss" @longtap="longClick(newsitem)">

							<view class="orderCont">
								<view class="checkAll" v-if="isCheckBtn&&tabIndex==5" @tap="clickChang(newsitem)">
									<view class="noCheck" v-if="!newsitem.isChecked"></view>
									<image src="../../static/images/books/xuanze.png" mode="" style="height: 38upx; width: 38upx;" v-if="newsitem.isChecked"></image>
								</view>
					...
					<!-- 暂无数据 -->
					<view class="noHave" v-if="dataType.length==0">
...
<script>
	...
	export default {
		...
		onLoad: function(option) {
			...
			console.log("shuju", this.$store.state.userId)
		...
		methods: {
			...
			getclassify() {
				...
				getFavoritesPageAPI(objMsg).then(res => {
					...
					this.loadingType = 0;
					if (this.dataType.length >= this.total[itemId + 'Total'] || this.total[itemId + 'Total'] <= 10) {
